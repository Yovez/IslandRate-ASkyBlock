	...
	public void onSignCreate(SignChangeEvent e) {
		if (e.getLine(0).equalsIgnoreCase("[islandrate]")) {
			...
			if (e.getLine(1).equalsIgnoreCase("menu")) {
				...
				if (addon.getAskyblock().getIslandAt(e.getBlock().getLocation()).getOwner() == null) {
				...
				if (addon.getAskyblock().getIslandAt(e.getBlock().getLocation()).getOwner() != null) {
					...
					e.setLine(1,
							addon.getMessage("sign.rate-island.line-1", e.getPlayer(),
									Bukkit.getOfflinePlayer(
											addon.getAskyblock().getIslandAt(e.getBlock().getLocation()).getOwner()),
									0, 0));
					e.setLine(2,
							addon.getMessage("sign.rate-island.line-2", e.getPlayer(),
									Bukkit.getOfflinePlayer(
											addon.getAskyblock().getIslandAt(e.getBlock().getLocation()).getOwner()),
									0, 0));
					e.setLine(3,
							addon.getMessage("sign.rate-island.line-3", e.getPlayer(),
									Bukkit.getOfflinePlayer(
											addon.getAskyblock().getIslandAt(e.getBlock().getLocation()).getOwner()),
									0, 0));
				} else {
			...
			if (e.getClickedBlock().getType().equals(Material.SIGN)
				...
				if (sign.getLine(0).equals("§8[§aIslandRate§8]")) {
					if (sign.getLine(1)
						...
						RateMenu menu = null;
						if (!addon.getAskyblock().getIslandAt(sign.getLocation()).isLocked()) {
							e.getPlayer().sendMessage(addon.getMessage("no-island", e.getPlayer(), null, 0, 0));
							return;
						}
						menu = new RateMenu(addon, Bukkit
								.getOfflinePlayer(addon.getAskyblock().getIslandAt(sign.getLocation()).getOwner()));
						...
						if (addon.getAskyblock().getIslandAt(sign.getLocation()).isLocked()) {
							if (sign.getLine(2)
									.equals(addon.getMessage("sign.rate-island.line-2", e.getPlayer(),
											Bukkit.getOfflinePlayer(
													addon.getAskyblock().getIslandAt(sign.getLocation()).getOwner()),
											0, 0))) {
